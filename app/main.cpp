...
#include <../include/algorithms/homography.h>
#include <constants.h>
...
using namespace cv;
int run_homography (const std::string& imagePath,
                    std::vector<int>& sourceCoords,
                    std::vector<int>& destinationCoords,
    ...
    std::array<float, Homographer::HOMOGRAPHY_SIZE> homography =
...
int setup_cli (CLI::App& app,
               std::string& homographyImagePath,
               std::vector<int>& homographySourceCoords,
               std::vector<int>& homographyDestinationCoords,
               std::string& homographyCoordsFilePath,
               std::string& homographyOutputImagePath) {
    // Homography subcommand
    auto homography_cmd =
    app.add_subcommand ("homography", "Run the homography algorithm");
    homography_cmd
    ->add_option ("-f,--file", homographyImagePath, "The image to run the algorithm on")
    ->required ();

    auto source_option =
    homography_cmd
    ->add_option ("-s, --source", homographySourceCoords,
                  "Source points for homography separated by spaces")
    ->expected (8);

    auto destination_option =
    homography_cmd
    ->add_option ("-d, --destination", homographyDestinationCoords,
                  "Destination points for homography separated by spaces")
    ->expected (8);

    homography_cmd
    ->add_option ("-c, --coords-file", homographyCoordsFilePath, "File containing comma seperated source and destination coordinates on each new line")
    ->excludes (source_option)
    ->excludes (destination_option);

    homography_cmd->add_option (
    "-o, --output-file", homographyOutputImagePath,
...
int main (int argc, char** argv) {
    ...
    std::vector<int> homographyDestinationCoords =
    DefaultConstants::DEFAULT_HOMOGRAPHY_DESTINATION_COORDS;
    std::string homographyCoordsFilePath;
    std::string homographyOutputImagePath;

    setup_cli (app, homographyImagePath, homographySourceCoords, homographyDestinationCoords,
               homographyCoordsFilePath, homographyOutputImagePath);

    CLI11_PARSE (app, argc, argv);

    auto homography_cmd = app.get_subcommand ("homography");
    if (homography_cmd && *homography_cmd) {
        return run_homography (homographyImagePath, homographySourceCoords,
